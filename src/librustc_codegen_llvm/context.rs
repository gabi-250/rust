...
impl<Value : ?Sized> ty::layout::HasDataLayout for CodegenCx<'ll, 'tcx, &'ll Value>
...
impl HasTargetSpec for CodegenCx<'ll, 'tcx, &'ll Value> {
    fn target_spec(&self) -> &Target {
        &self.tcx.sess.target.target
    }
}

impl ty::layout::HasTyCtxt<'tcx> for CodegenCx<'ll, 'tcx, &'ll Value> {
    fn tcx<'b>(&'b self) -> TyCtxt<'b, 'tcx, 'tcx> {
        self.tcx
    }
}

impl LayoutOf for CodegenCx<'ll, 'tcx, &'ll Value> {
    type Ty = Ty<'tcx>;
